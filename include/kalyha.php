...
//	    $szumszam[dhs][$i] = round($szam[$i][14], $this->tizedes);
	    $szam[$i][15] = pow(2.718, (-.83 * $szam[$i][11] / $fustjarathossz)) * 550; // t - pont
	    $szumszam[tp][$i] = round($szam[$i][15], $this->tizedes);
	    $szam[$i][16] = pow(2.718, (-.83 * $szam[$i][12] / $fustjarathossz)) * 550; // t - szakasz
...
//	    $szumszam[lam][$i] = round($szam[$i][25], $this->tizedes);
	...
	$szumma[hom] = round(pow(2.718, (-.83 * $L / $fustjarathossz)) * 550, $this->picitizedes);
	$osszpharray = array();
	$osszpsarray = array();
	$osszpaarray = array();
	$osszellarray = array();
	...
	$szumma[hatasfok] = round(101.09 - (0.0941 * pow(2.718, (-.83 * $L / $fustjarathossz)) * 550) 
			- (6.275 * pow(10, -6) * pow(pow(2.718, (-.83 * $L / $fustjarathossz)) * 550, 2))
			- (3.173 * pow(10, -9) * pow(pow(2.718, (-.83 * $L / $fustjarathossz)) * 550, 3)), $this->tizedes);
